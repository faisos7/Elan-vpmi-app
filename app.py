...
start_date_raw = str(row.get('시작일', '')).strip()
db[name] = {
"group": row.get('그룹', ''), "note": row.get('비고', ''),
"default": True if str(row.get('기본발송', '')).upper() == 'O' else False,
"items": items_list, "round": round_num, "start_date_raw": start_date_raw
}
return db
except:
return {}
...
return pd.DataFrame()
# ==============================================================================
# 4. 데이터 초기화 및 세션 상태 (init_session_state)
# ==============================================================================
...
if save_to_history(records): st.success("저장 및 재고 반영 완료!")
...
rec = [batch_id, datetime.now(KST).strftime("%Y-%m-%d"), target_product, "우유+스타터", f"{milk_kg:.1f}", "스타터배합", 0, 0, "", status_json]
...
st.rerun()
st.subheader("🌡️ 2단계: 대사 관리 및 분리")
if st.button("🔄 상태 새로고침"): st.rerun()
prod_df = load_sheet_data("curd_prod", "생산일")
if not prod_df.empty:
for idx, row in prod_df.iterrows():
try: status = json.loads(row['상태'])
except: continue
if status.get('meta', 0) == 0 and status.get('sep', 0) == 0: continue
with st.container(border=True):
st.markdown(f"**[{row['배치ID']}] {row['종류']}**")
if st.button("분리/포장 업데이트", key=f"upd_{row['배치ID']}"):
st.info("단계별 업데이트 로직은 원본 v.0.9.8에 따라 수행됩니다.")
with t7:
...
new_n = st.text_input("처방명"); new_c = st.text_area("처방 내용")
if st.form_submit_button("등록"):
if new_n: st.session_state.regimen_db[new_n] = new_c; st.rerun()
else:
st.info(f"📋 **{selected_regimen}**")
st.text_area("내용", value=st.session_state.regimen_db[selected_regimen], height=200, disabled=True)
with t9:
st.header("🏭 기타 생산 이력")
with st.container(border=True):
st.subheader("📝 생산 기록 입력")
c1, c2, c3 = st.columns(3)
p_dt = c1.date_input("생산일", datetime.now(KST))
p_tp = c2.selectbox("종류", ["저염김치(0.3%)", "무염김치(0%)", "기타"])
p_nm = c3.selectbox("원재료명", st.session_state.raw_material_list)
if st.button("💾 생산 기록 저장"):
bid = f"{p_dt.strftime('%y%m%d')}-{p_nm}-{uuid.uuid4().hex[:4]}"
if save_production_record("other_prod", [bid, p_dt.strftime("%Y-%m-%d"), p_tp, p_nm, 1.0, "1:8", 0, 0, "", "진행중"]):
st.success("저장 완료!")
with t10:
st.header("🔬 대사 관리 및 pH 측정")
with st.container(border=True):
c1, c2 = st.columns(2)
ph_dt = c1.date_input("측정일", datetime.now(KST))
ph_tm = c2.time_input("측정시간", datetime.now(KST).time())
c3, c4 = st.columns(2)
ph_v = c3.number_input("pH 값", 0.0, 14.0, 5.0, step=0.01)
ph_t = c4.number_input("온도 (℃)", 0.0, 50.0, 30.0)
if st.button("💾 pH 저장"):
dt_s = f"{ph_dt.strftime('%Y-%m-%d')} {ph_tm.strftime('%H:%M')}"
if save_ph_log(["DIRECT", dt_s, ph_v, ph_t, ""]): st.success("저장됨!")
