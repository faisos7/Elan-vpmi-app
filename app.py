...
def get_week_info(date_obj):
    """날짜를 입력받아 'M월 N주' 문자열 반환"""
    month = date_obj.month
    # 간단한 주차 계산 (1일~7일=1주, 8일~14일=2주...)
...
if mode == "👤 신규 환자 등록" or mode == "🧪 신규 레시피 등록":
    st.title(mode)
    st.info("이전 코드를 참고하거나 필요시 요청해주세요.")
...
# ==========================================
    ...
    t1, t2, t3, t4, t5, t6 = st.tabs(t_labels)
    ...
    # [v.4.3.1] Tab 5: 생산 관리 (주차 선택 기능 추가)
    with t5:
        st.header(f"🏭 생산 관리 ({week_str} 작업)")
        ...
        st.caption(f"현재 선택: {sel_sim_week} 기준 생산 계획")
